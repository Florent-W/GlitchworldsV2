...
final class AdministrationExportController extends AbstractController
{
    ...
    public function exporter(string $type, Connection $db): StreamedResponse
    {
        ...
        $reponse = new StreamedResponse(static function () use ($db, $sql, $entetes): void { $sortie = fopen('php://output', 'wb'); fwrite($sortie, "\xEF\xBB\xBF"); fputcsv($sortie, $entetes, ';'); foreach ($db->iterateAssociative($sql) as $ligne) { $valeurs = array_map(static fn (mixed $valeur): mixed => is_string($valeur) && preg_match('/^[=+\-@]/', $valeur) ? "'".$valeur : $valeur, array_values($ligne)); fputcsv($sortie, $valeurs, ';'); } fclose($sortie); });
